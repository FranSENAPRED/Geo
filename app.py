...
# Simular los datos de simulación del Ministerio de Obras Públicas de Chile (MOP)
np.random.seed(42)
...
app.layout = html.Div([
    ...
    html.Div(id='dashboard-div', children=[
        ...
        html.P("Aquí podrá explorar los datos filtrando por año y por región. Los gráficos se actualizarán automáticamente según su selección, mostrándole de forma visual cómo se distribuye la inversión del MOP.",
        ...
        dcc.Graph(id='linea-inversion-anual'),
        dcc.Graph(id='area-regiones'),
...
def update_visuals(año, region=None):
    ...
    node_trace = go.Scatter(
        ...
        text=[f"{node} - conexiones: {G.degree(node)} - Monto: ${int(df_year[df_year['Región']==node]['Monto (MM$)'].sum()) if node in df_year['Región'].values else int(df_year[df_year['Tipo de Obra']==node]['Monto (MM$)'].sum())}" for node in G.nodes],
        hoverinfo='text',
        showlegend=True,
        name='Nodos')

    fig_network = go.Figure(data=[edge_trace, node_trace])
    fig_network.update_layout(title='Relación entre Regiones y Tipos de Obra (Red)', height=500)
    ...
    linea.update_traces(line=dict(color='royalblue'), marker=dict(size=6))
    linea.update_layout(height=400)
    ...
    mapa.update_geos(fitbounds="locations", visible=True, showcountries=True)
